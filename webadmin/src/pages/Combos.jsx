...
import './Combos.css';
import { comboAPI } from '../api/apiService.js';
...
const Combos = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCombo) {
        await comboAPI.update(editingCombo._id, formData);
      } else {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Xóa combo/sản phẩm này?')) return;
    try {
          ...
          <div className="modal-content card glass animate-scale-in" style={{maxWidth: 600}}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group mb-md">
                ...
                {formData.image && (
                  <img src={formData.image} alt="preview" className="mt-sm rounded" style={{maxHeight: 160, objectFit: 'cover'}} />
                )}
              </div>
