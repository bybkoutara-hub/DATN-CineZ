...
export const reviewAPI = {
  ...
  updateStatus: async (id, status) => {
    ...
    return formatResponse(response.data.data || response.data, response.data.message || 'Cập nhật trạng thái thành công');
  },
  delete: async (id) => {
  ...
  blockUser: async (userId) => {
    const response = await apiClient.put(`/users/${userId}/block-comment`);
    return formatResponse(response.data.data || response.data, response.data.message || 'Chặn người dùng thành công');
  },
  unblockUser: async (userId) => {
    const response = await apiClient.put(`/users/${userId}/unblock-comment`);
    return formatResponse(response.data.data || response.data, response.data.message || 'Bỏ chặn người dùng thành công');
  },
};
