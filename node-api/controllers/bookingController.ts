...
export const createBooking = async (req: any, res: Response): Promise<void> => {
  try {
    ...
    // Giới hạn tối đa 8 ghế cho mỗi lần đặt vé.
    if (seats.length > 8) {
      res.status(400).json({
        success: false,
        message: "Mỗi lần đặt vé chỉ được chọn tối đa 8 ghế.",
      });
      return;
    }

    // Không cho gửi trùng ghế từ client.
    if (new Set(seats.map((seat: any) => String(seat))).size !== seats.length) {
      res.status(400).json({
        success: false,
        message: "Danh sách ghế không hợp lệ vì có ghế bị trùng.",
      });
      return;
    }
