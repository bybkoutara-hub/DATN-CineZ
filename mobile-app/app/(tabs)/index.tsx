...
const SectionHeader = ({ title }: { title: string }) => (
  <View style={styles.sectionHeader}>
    ...
    <TouchableOpacity activeOpacity={0.7} style={styles.seeAllBtn}>
      ...
      <Feather name="chevron-right" size={14} color={PRIMARY_YELLOW} style={{ marginLeft: 2 }} />
    </TouchableOpacity>
  </View>
);

const ServiceItem = ({ title, iconImage }: { title: string; iconImage: any }) => (
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.searchContainer}>
          ...
          {searchQuery ? (
            <>
              <TouchableOpacity activeOpacity={0.7} onPress={handleSearch} style={styles.filterIconBtn}>
                <Feather name="search" size={18} color={PRIMARY_YELLOW} />
              </TouchableOpacity>
              <TouchableOpacity activeOpacity={0.7} onPress={() => { setSearchQuery(""); router.push({ pathname: "/(tabs)/movie", params: { search: "" } }); }} style={{ marginLeft: 8 }}>
                <Feather name="x" size={18} color="#999" />
              </TouchableOpacity>
            </>
          ) : (
            <TouchableOpacity activeOpacity={0.7} style={styles.filterIconBtn} onPress={handleSearch}>
              <Feather name="search" size={18} color={PRIMARY_YELLOW} />
            </TouchableOpacity>
          )}
        </View>
        ...
        {nowPlaying.length === 0 ? (
          ...
          <View>
            <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} decelerationRate="fast" snapToInterval={SCREEN_WIDTH}
              onMomentumScrollEnd={(e) => {
                setActivePageIndex(Math.round(e.nativeEvent.contentOffset.x / SCREEN_WIDTH));
              }}
            >
              {nowPlaying.map((movie, index) => (
                <View key={movie._id || index} style={{ width: SCREEN_WIDTH, alignItems: "center" }}>
                  ...
                  >
                    ...
                    <View style={styles.ratingRow}>
                      ...
                      <Text style={styles.ratingCount}> ({movie.total_reviews || "0"})</Text>
                    </View>
                  </TouchableOpacity>
                </View>
              ))}
            ...
            <View style={styles.paginationDotsContainer}>
              {nowPlaying.map((_, i) => (
                <View key={i} style={[styles.dot, activePageIndex === i && styles.dotActive]} />
              ))}
            </View>
          </View>
        )}
            ...
            {comingSoon.map((movie, index) => (
              ...
              >
                ...
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoText} numberOfLines={1}>{movie.genres?.join(", ") || "Khoa học viễn tưởng"}</Text>
                </View>
                <View style={styles.infoRow}>
                  <Ionicons name="calendar-outline" size={12} color="#999999" />
                  <Text style={styles.infoText}>{movie.release_date ? new Date(movie.release_date).toLocaleDateString("vi-VN") : "Sắp chiếu"}</Text>
                </View>
              </TouchableOpacity>
            ))}
          </ScrollView>
        ...
        <TouchableOpacity activeOpacity={0.9} style={styles.promoCard}>
          <Image source={require("../../assets/images/PromoDiscount.png")} style={styles.promoImage} />
        </TouchableOpacity>

        {/* SERVICE */}
        <SectionHeader title="Dịch vụ" />
        <View style={styles.serviceRow}>
          <ServiceItem title="Thuê phim" iconImage={require("../../assets/images/rental.png")} />
          <ServiceItem title="Imax" iconImage={require("../../assets/images/imax.png")} />
          <ServiceItem title="4DX" iconImage={require("../../assets/images/4dx.png")} />
          <ServiceItem title="Sweetbox" iconImage={require("../../assets/images/sweetbox.png")} />
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalList} contentContainerStyle={styles.horizontalListContent}>
          <View style={styles.newsCard}>
            <Image source={{ uri: "https://images.unsplash.com/photo-1509347528160-9a9e33742cdb?w=500" }} style={styles.newsImage} />
            <Text style={styles.newsTitle} numberOfLines={2}>Lộ thời điểm The Batman 2 bắt đầu bấm máy</Text>
          </View>
          <View style={styles.newsCard}>
            <Image source={{ uri: "https://images.unsplash.com/photo-1611604548018-d56bbd85d681?w=500" }} style={styles.newsImage} />
            <Text style={styles.newsTitle} numberOfLines={2}>6 trận chiến hoành tráng nhất của Hulk trong MCU</Text>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  greetingText: { color: "#888888", fontSize: 14, marginBottom: 2, fontWeight: "400" },
  welcomeText: { color: "#ffffff", fontSize: 24, fontWeight: "700" },
  notificationBtn: { position: "relative", padding: 2 },
  notificationDot: { position: "absolute", top: 2, right: 2, width: 8, height: 8, backgroundColor: "#4CD964", borderRadius: 4 },
  ...
  ratingScore: { color: PRIMARY_YELLOW, fontSize: 14, fontWeight: "700" },
  ratingCount: { color: "#666666", fontSize: 13 },
  paginationDotsContainer: { flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 6, marginTop: 4, marginBottom: 24 },
  dot: { width: 6, height: 6, backgroundColor: "#333333", borderRadius: 3 },
  dotActive: { backgroundColor: PRIMARY_YELLOW, width: 18 },
  ...
  promoCard: { marginHorizontal: 24, borderRadius: 24, overflow: "hidden", marginBottom: 28, aspectRatio: 327 / 167, width: SCREEN_WIDTH - 48, alignSelf: "center" },
  promoImage: { width: "100%", height: "100%", resizeMode: "cover" },
  serviceRow: { flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 24, marginBottom: 28 },
  serviceItem: { alignItems: "center", width: 72 },
  serviceIconCircle: { width: 60, height: 60, borderRadius: 30, overflow: "hidden", marginBottom: 8 },
  serviceCustomImage: { width: "100%", height: "100%", resizeMode: "cover" },
  serviceTitle: { color: "#ffffff", fontSize: 12, fontWeight: "500" },
  newsCard: { width: 240 },
  newsImage: { width: "100%", height: 130, borderRadius: 10, marginBottom: 10 },
  newsTitle: { color: "#ffffff", fontSize: 14, fontWeight: "600", lineHeight: 20 },
  emptyText: { color: "#666666", fontSize: 14, textAlign: "center", marginVertical: 20, fontStyle: "italic" },
});