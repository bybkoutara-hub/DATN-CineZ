...
const formatVnd = (value) => `${(value || 0).toLocaleString("vi-VN")} đ`;
...
const buildEmailHtml = (data) => {
    const comboRows = data.combos.length
        ? data.combos
            .map((c) => `<tr><td style="padding:4px 0;color:#444;">${c.name} × ${c.quantity}</td>
...
const sendBookingConfirmationEmail = async (bookingId) => {
    try {
        ...
        const booking = await bookingModel_js_1.default.findById(bookingId)
